...
print(classroom[0] == 90) # False / 80 넣으면 True
# 클래스 변수 호출
print(Student.count) # 클래스 변수는 인스턴스 없이 호출 가능 
...
class Circle:
    ...
    # 게터와 세터, 프라이빗 변수를 외부에서 간접 접근 허용 함수
    @property
    def radius(self):
...
Circle(20) # 인스턴스 생성후 변수에 저장하지 않았으므로 GC(갈비지컬렉터)가 자동으로 인스턴스 삭제
circle = Circle(10)
#print(circle.radius) # 프라이빗 변수 이므로 직접 호출시 오류 발생

print(circle.radius) # 간접접근, getter 10
circle.radius = 20 # 간접접근, setter
print(circle.radius) # 간접접근, getter 20
...
class Parent:
    def __init__(self):
        self.value = "테스트"
        print("부모 클래스의 생성자가 호출")
    def test(self):
        print("부모 클래스의 test()메소드")

class Child(Parent): # class 클래스명(상위 클래스명):
    def __init__(self):
...
child.test()
print(child.value) # 자식은 부모의 멤버변수와 멤버함수 사용할 수 있다. <물려받음>


# 상속 이용한 나만의 예외 클래스 만들기
