...
# 인덱스를 ['월', '화', '수']로 지정하여 생성하고 그 결과를 출력하시오. 

# (발생하는 현상과 이유를 주석으로 설명하시오.)
sales = {'mon': 100, 'tue': 200, 'wed': 300}
x = pd.Series(sales, index=['월', '화', '수'])
print(x)
...
x = data.sort_index().sort_values(ascending=False)
# x = data.sort_index().sort_values(ascending=False, kind='stable') # 값 내림차순 1차 정렬 한것을 유지하고 2차 정렬 하는 방법 
print("7번", x)
...
y = data.groupby(level=0).agg(['sum','mean'])
print(y)
